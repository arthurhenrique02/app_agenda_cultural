...
# Parse arguments
TOOL="claude"  # Default to Claude for this repository setup
MAX_ITERATIONS=10
...
if [[ "$TOOL" == "amp" ]] && ! command -v amp >/dev/null 2>&1; then
  echo "Error: amp CLI not found. Install amp or use --tool claude."
  exit 1
fi

if [[ "$TOOL" == "claude" ]] && ! command -v claude >/dev/null 2>&1; then
  echo "Error: claude CLI not found. Install @anthropic-ai/claude-code or use --tool amp."
  exit 1
fi
...
if [ ! -f "$PRD_FILE" ]; then
  echo "Error: $PRD_FILE not found. Create it from prd.json.example before running Ralph."
  exit 1
fi
